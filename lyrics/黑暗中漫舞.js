// 歌曲：黑暗中漫舞

(function() {
    const song = {
        id: 28,
        title: "黑暗中漫舞",
        titleJyutping: ["hak1","am3","zung1","maan6","mou5"],
        artist: "陈奕迅",
        artistJyutping: ["can4","jik6","seon3"],
        lyricist: "林夕",
        lyricistJyutping: ["lam4","zik6"],
        composer: "陈小霞",
        composerJyutping: ["can4","siu2","haa4"],
        lyrics: [
            { chars: ["为","何","未","能","学","会","起","舞","便","已","抱","紧","你"], jp: ["wai4","ho4","mei6","nang4","hok6","wui6","hei2","mou5","bin6","ji5","pou5","gan2","nei5"] },
            { chars: ["谁","料","到","资","质","不","配","合","你"," ","左","脚","举","起","了","便","要","别","离"], jp: ["seoi4","liu6","dou3","zi1","zat1","bat1","pui3","hap6","nei5","","zo2","goek3","geoi2","hei2","liu5","bin6","jiu3","bit6","lei4"] },
            { chars: ["为","何","未","能","让","我","衰","老","便","要","放","开","你"], jp: ["wai4","ho4","mei6","nang4","joeng6","ngo5","seoi1","lou5","bin6","jiu3","fong3","hoi1","nei5"] },
            { chars: ["陪","你","跳","通","宵","都","够","力","气"," ","请","鉴","别","姿","态","美","不","美"], jp: ["pui4","nei5","tiu3","tung1","siu1","dou1","gau3","lik6","hei3","","cing2","gaam3","bit6","zi1","taai3","mei5","bat1","mei5"] },
            { paragraphBreak: true },
            { chars: ["学","跳","舞"," ","有","福","气"," ","手","差","点","扑","地"], jp: ["hok6","tiu3","mou5","","jau5","fuk1","hei3","","sau2","caa1","dim2","pok3","dei6"] },
            { chars: ["犹","如","自","卑","水","银","泻","地"], jp: ["jau4","jyu4","zi6","bei1","seoi2","ngan4","se3","dei6"] },
            { chars: ["学","爱","你"," ","美","不","美"," ","天","黑","黑","到","地"], jp: ["hok6","ngoi3","nei5","","mei5","bat1","mei5","","tin1","hak1","hak1","dou3","dei6"] },
            { chars: ["朝","着","坏","了","的","灯","泡","舞","着","别","离"], jp: ["ciu4","zoek6","waai6","liu5","dik1","dang1","paau1","mou5","zoek6","bit6","lei4"] },
            { chars: ["其","实","地","球","没","有","你"," ","站","到","虚","脱","便","会","飞"], jp: ["kei4","sat6","dei6","kau4","mut6","jau5","nei5","","zaam6","dou3","heoi1","tyut3","bin6","wui6","fei1"] },
            { chars: ["何","必","怪","责","双","脚","未","够","伶","俐"," ","不","比","你","优","美"], jp: ["ho4","bit1","gwaai3","zaak3","soeng1","goek3","mei6","gau3","ling4","lei6","","bat1","bei2","nei5","jau1","mei5"] },
            { paragraphBreak: true },
            { chars: ["为","何","未","能","待","我","差","到","让","我","去","憎","你"], jp: ["wai4","ho4","mei6","nang4","doi6","ngo5","caa1","dou3","joeng6","ngo5","heoi3","zang1","nei5"] },
            { chars: ["捱","到","这","伤","口","激","发","士","气"," ","左","脚","高","举","到","白","发","齐","眉"], jp: ["ngaai4","dou3","ze5","soeng1","hau2","gik1","faat3","si6","hei3","","zo2","goek3","gou1","geoi2","dou3","baak6","faat3","cai4","mei4"] },
            { chars: ["为","何","未","能","待","我","好","到","没","法","舍","得","你"], jp: ["wai4","ho4","mei6","nang4","doi6","ngo5","hou2","dou3","mut6","faat3","se3","dak1","nei5"] },
            { chars: ["顽","固","的","车","軚","追","了","万","里"," ","先","发","现","一","早","泄","光","气"], jp: ["waan4", "gu3", "dik1", "ce1", "taai5", "zeoi1", "liu5", "maan6", "lei5", "", "sin1", "faat3", "jin6", "jat1", "zou2", "sit3", "gwong1", "hei3"] },
            { paragraphBreak: true },
            { chars: ["学","爱","你"," ","美","不","美"," ","天","黑","黑","到","地"], jp: ["hok6","ngoi3","nei5","","mei5","bat1","mei5","","tin1","hak1","hak1","dou3","dei6"] },
            { chars: ["朝","着","坏","了","的","灯","泡","舞","着","别","离"], jp: ["ciu4","zoek6","waai6","liu5","dik1","dang1","paau1","mou5","zoek6","bit6","lei4"] },
            { chars: ["其","实","地","球","没","有","你"," ","站","到","虚","脱","便","会","飞"], jp: ["kei4","sat6","dei6","kau4","mut6","jau5","nei5","","zaam6","dou3","heoi1","tyut3","bin6","wui6","fei1"] },
            { chars: ["何","必","怪","责","双","脚","未","够","伶","俐"," ","不","比","你","优","美"], jp: ["ho4","bit1","gwaai3","zaak3","soeng1","goek3","mei6","gau3","ling4","lei6","","bat1","bei2","nei5","jau1","mei5"] },
            { paragraphBreak: true },
            { chars: ["为","何","未","曾","让","我","得","够","便","要","损","失","你"], jp: ["wai4","ho4","mei6","zang1","joeng6","ngo5","dak1","gau3","bin6","jiu3","syun2","sat1","nei5"] },
            { chars: ["捱","到","那","一","天","参","透","佛","理"," ","手","会","松","灯","会","灭","有","限","期"], jp: ["ngaai4","dou3","naa5","jat1","tin1","caam1","tau3","fat6","lei5","","sau2","wui6","cung4","dang1","wui6","mit6","jau5","haan6","kei4"] },
            { chars: ["为","何","未","能","让","我","死","去","便","要","认","识","你"], jp: ["wai4","ho4","mei6","nang4","joeng6","ngo5","sei2","heoi3","bin6","jiu3","jing6","sik1","nei5"] },
            { chars: ["难","道","你","很","想","天","使","问","我"," ","我","的","舞","步","跳","得","可","美"], jp: ["naan4","dou6","nei5","han2","soeng2","tin1","si2","man6","ngo5","","ngo5","dik1","mou5","bou6","tiu3","dak1","ho2","mei5"] }
        ]
    };
    if (typeof window !== 'undefined' && window.__songs) {
        window.__songs.push(song);
    }
})();
